...
@file commando.py
@author Naval Group
@brief Defines the Commando vessel agent class for simulation.
@version 0.1
@date 2025-10-08
...
class Commando(Agent):
    """
    Commando vessel agent in the simulation.

    Inherits from the base Agent class and initializes the Commando-specific
    ...
    def __init__(self, sdf_string: str, world_name: str, xdyn_enabled: bool):
        ...
        self.model_name = "commando"
        self.renderer_type_name = self.model_name
        if xdyn_enabled:
            self.xdyn_port = 12352
    ...
    def lotus_param(self) -> str:
        """
        Generate the LOTUSim-compatible parameter string for Commando.
