"""
@file bluerov2_heavy.py
@author Naval Group
@brief Defines the BlueROV agent class for simulation.
@version 0.1
@date 2025-10-08
...
class Bluerov2Heavy(Agent):
    """
    BlueROV2Heavy agent in the simulation.

    Inherits from the base Agent class and initializes the BlueROV2-specific
    parameters such as its SDF model and world.
    """
    ...
    def __init__(self, sdf_string: str, world_name: str, xdyn_enabled: bool):
        ...
        self.model_name = "bluerov2_heavy"
        self.renderer_type_name = self.model_name
        if xdyn_enabled:
            self.xdyn_port = 12347
            self.xdyn_ip = "127.0.0.1"
        else:
            self.xdyn_port = None
            self.xdyn_ip = None
        self.domains = ["Underwater"]
    ...
    def lotus_param(self) -> str:
        """
        Generate the LOTUSim-compatible parameter string for BlueROV2Heavy.
