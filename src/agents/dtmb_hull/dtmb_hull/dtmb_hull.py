...
@file dtmb_hull.py
@author Naval Group
@brief Defines the Dtmb vessel agent class for simulation.
@version 0.1
@date 2025-10-08
...
class DtmbHull(Agent):
    """
    DtmbHull surface vessel agent in the simulation.

    Inherits from the base Agent class. Initializes Dtmb-specific parameters,
    including its SDF model and world.

    Note: This agent does not have a corresponding 3D model in Unity and
    therefore cannot be rendered in Unity.
    """
    ...
    def __init__(self, sdf_string: str, world_name: str, xdyn_enabled: bool):
        ...
        self.model_name = "dtmb_hull"
        self.renderer_type_name = self.model_name
        if xdyn_enabled:
            self.xdyn_port = 12345
    ...
    def lotus_param(self) -> str:
        """
        Generate the LOTUSim-compatible parameter string for Dtmb_hull.
