...
@file wamv.py
@author Naval Group
@brief Defines the Wamv agent class for simulation.
@version 0.1
@date 2025-10-08
...
class Wamv(Agent):
    """
    Wamv agent in the simulation.

    Inherits from the base Agent class and initializes the Wamv-specific
    ...
    def __init__(self, sdf_string: str, world_name: str, xdyn_enabled: bool):
        """
        Initializes a WAM-V surface vessel agent.
    ...
    def lotus_param(self) -> str:
        """
        Generate the LOTUSim-compatible parameter string for Wamv.
